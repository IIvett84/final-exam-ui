...
function createEl(tag, options) {
  ...
  function createBookCard(cardNum, title, sub, text) {
    ...
    const bookSub = createEl("p", { className: "beer-sub", innerText: sub });
    const bookTitle = createEl("h1", { className: "beer-title", innerText: title });
    const bookText = createEl("p", { className: "beer-text", innerText: text });
    ...
    cardText.append(cardNumEl, bookSub, bookTitle,  bookText);
    cardDiv.append(cardText, button);
  
    return cardDiv;
  }

  function createRoot(booksData) {
    const headerEl = createEl("header", { className: "header-el" });
    const beersEl = createEl("h2", { className: "best-books", innerText: booksData.logo });
    ...
    const cards = booksData.cards;
    for (let i = 0; i < cards.length; i++) {
      const cardData = cards[i];
      const bookCard = createBookCard(i + 1, cardData.title, cardData.sub, cardData.text);
